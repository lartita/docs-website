...
In order to make migration from earlier versions as easy as possible, we've developed a compatibility layer that will translate most of the options from the old newrelic-infrastructure chart to their new counterparts. This compatibility layer is temporary and will be removed in the future, so we encourage you to carefully read this guide and migrate the configuration with caution.
